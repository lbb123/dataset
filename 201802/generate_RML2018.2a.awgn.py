...
for snr in snr_vals:
    ...
    for alphabet_type in transmitters.keys():
        for i,mod_type in enumerate(transmitters[alphabet_type]):
	      ...
	      #chan = channels.dynamic_channel_model( 200e3, 0, 0, 0, 0, 8, 1, False, 4.0, (0, ), (1, ), ntaps, noise_amp, seed=2018 )
	      ...
	      add = blocks.add_vcc(1)
              noise = analog.noise_source_c(analog.GR_GAUSSIAN, noise_amp, seed=2018)
...
cPickle.dump( dataset, file("RML2018.2a_awgn.dat", "wb" ) )
